...
def IntegratedKalmanFilter(kernel, t_states, y, obsid, instid, stateid, noise, return_v_S=False):
    '''
    ...
    output = integrated_kalman_filter(A_aug, Q_aug, H_aug, RESET,
                                      t_states, y, obsid, instid, stateid, R,
                                      m0, P0)
    if return_v_S:
        return output
    else:
...
def integrated_kalman_filter(A_aug, Q_aug, H_aug, RESET,
    ...
    K = len(t_states) # number of iterations (2N)
    N = int(K/2)
