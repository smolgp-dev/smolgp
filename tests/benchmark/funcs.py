...
######################################## VALUE-AND-GRAD VARIANTS ########################################
# The likelihood as a hyperparameter fit actually evaluates it: value *and*
# gradient with respect to the kernel's parameters. Reverse-mode changes the
# cost profile completely -- it has to keep the forward pass's intermediates --
# so it is benchmarked as its own sweep rather than inferred from the forward
# numbers.
#
# eqx.filter_value_and_grad differentiates only the inexact array leaves, so a
# kernel's static fields (its name, its instrument count) are left alone.
#
# These are written out one by one, rather than generated by a decorator,
# because the profiler pickles the function to hand it to a spawned subprocess.
# A closure returned by a decorator either fails to pickle or -- if __name__ is
# rewritten to match the wrapped function -- resolves on the other side to the
# *forward* version, which would silently benchmark the wrong thing.


def ss_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: ss_llh(data, k))(kernel)


def qs_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: qs_llh(data, k))(kernel)


def gp_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: gp_llh(data, k))(kernel)


def pss_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: pss_llh(data, k))(kernel)


def pqs_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: pqs_llh(data, k))(kernel)


def iss_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: iss_llh(data, k))(kernel)


def igp_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: igp_llh(data, k))(kernel)


def ipss_llh_vg(data, kernel):
    return eqx.filter_value_and_grad(lambda k: ipss_llh(data, k))(kernel)
