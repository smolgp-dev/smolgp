...
# Scratch directory for plots generated by --quick runs 
SCRATCH_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scratch_figures")
...
# THEORY_MEM / THEORY_MEM_INT below are used only by the optional `show_theory`
# diagnostic, which draws the derived laws alongside the measurements so the two
# can be compared. The figures' substituted points no longer come from them --
# see extrapolate_below_floor. Several entries have since drifted from what the
# code does (llh SSM 88 against ~170 B/N measured, cond SSM 184 against ~373,
# and pred GP 2400 against ~1362, which never agreed), so read a mismatch as a
# question about the derivation, not as a fault in the measurement.
...
def extrapolate_below_floor(Ns, measured, power, floor=MEM_FLOOR_BYTES):
    """Fill unresolvable memory points by continuing the curve's own measurements.

    Returns ``(values, was_substituted)``. A point is substituted when it was
    measured but carries no information: non-positive, meaning RSS saw no change
    at all, or under ``floor``, where what it reports is allocator noise rather
    than the allocation. NaN is left alone -- that means the point was never run
    (a size cutoff retired it, or it crashed), and inventing a value there would
    draw a curve the sweep never measured.

    The curve's exponent is already known -- ``power`` is the same one the
    panel's dotted continuation uses -- so only the coefficient is unknown, and
    it is fitted to every point that *is* above the floor::

        log c = mean(log y_i - power * log N_i)

    Fitting over all reliable points, rather than anchoring on the smallest of
    them, stops one noisy measurement from shifting the whole substituted
    section; sharing the exponent with the continuation keeps a curve's inward
    and outward extensions consistent with each other.

    This replaces a table of hardcoded B/N constants. That table was a
    measurement of one version of the code and went stale silently as soon as the
    implementation changed -- the split-scan likelihood moved SSM from 88 to
    ~170 B/N, leaving the substituted points on a line the measured ones no
    longer joined. Deriving the coefficient from the data makes the figure
    self-maintaining, and it is the honest thing to draw: these points are an
    extension of *this* sweep, not of an earlier one.
    """
    x = np.asarray(Ns, dtype=float)
    y = np.array([float(m) for m in measured], dtype=float)
    n = min(len(x), len(y))
    x, y = x[:n], y[:n]

    usable = np.isfinite(y) & (y > floor) & (x > 0)
    bad = np.isfinite(y) & ((y <= 0) | (y < floor))
    if not usable.any() or not bad.any():
        return y, np.zeros(n, dtype=bool)

    logc = float(np.mean(np.log(y[usable]) - power * np.log(x[usable])))
    return np.where(bad, np.exp(logc) * x**power, y), bad
...
SPECS = {
    ("llh", False): {
        ...
        "ylim2": (1e6, 1e12),
        # Pinned rather than taken from the data, so an abridged (--quick) run
        # draws the same axes as the production one and the two figures can be
        # laid side by side. A no-op for production, whose grid ends at 1e7.
        "xlim": (10, 1e7),
        "annotations1": [
            ("$N^3$", "GP"),
            ("$N$", "SSM"),
...
def make_benchmark_figure(
    ...
    Args:
        ...
        title_suffix: appended to the legend title, so a variant sweep's figure
            says what it measured rather than looking like the standard one.
        scratch: write into :data:`SCRATCH_DIR` instead of the published
            :data:`STATIC_DIR`. Set for abridged runs.
        tag: inserted before ``_benchmark`` in the filename, naming a variant
            *family* of results, e.g. ``"_value_and_grad"``.
        suffix: appended after ``_benchmark``, marking how a run was made rather
            than what it measured, e.g. ``"_quick"``. Keeps an
            abridged run's figure from overwriting the production one, which
            shares every other part of the name.
    ...
    if savefig is True:
        outdir = SCRATCH_DIR if scratch else STATIC_DIR
        os.makedirs(outdir, exist_ok=True)
        savefig = os.path.join(
            outdir,
            f"{kind}{'_int' if integrated else ''}{tag}_benchmark{suffix}.png",
        )
    ...
    # XLA's buffer accounting, slot [3] of each memory entry: the computation's
    # working set, measured and exact at every size. Extracted *before* the
    # substitution below, which rewrites entries as 2-tuples and would discard
    # it. Curves that have it need no theoretical stand-in -- the faded curve
    # says the same thing from measurement -- so they are excluded from the
    # substitution rather than getting both.
    # The solid curve answers "how much must be free to run this", so it uses
    # the absolute peak -- interpreter and runtime included -- wherever that has
    # been measured. Points predating that column fall back to the historical
    # peak-above-baseline, which is the same quantity minus a constant, so a
    # partially filled sweep degrades gracefully rather than mixing scales
    # invisibly: the fallback is always the smaller number.
    for name in list(memory):
        memory[name] = [
            ((p[2], p[1]) + tuple(p[2:]) if len(p) > 2 and p[2] == p[2]
             else tuple(p))
            for p in memory[name]
        ]

    mem_xla = {}
    for name in list(memory):
        vals = np.array(
            [p[3] if len(p) > 3 else np.nan for p in memory[name]], dtype=float
        )
        if np.isfinite(vals).any():
            mem_xla[name] = vals

    # Where the faded curve carries the small-N end, the solid one should stop
    # rather than be propped up: a sub-floor RSS reading is noise, and a zero
    # plotted on a log axis drops the line off the bottom of the panel. Masking
    # to NaN leaves a gap, so the solid curve simply begins where the
    # measurement becomes meaningful.
    for name in mem_xla:
        memory[name] = [
            ((np.nan, np.nan) + tuple(p[2:]) if not (p[0] > MEM_FLOOR_BYTES)
             else tuple(p))
            for p in memory[name]
        ]
    ...
    if substitute_theory:
        for name in list(memory):
            if name in mem_xla:
                continue
            # Extrapolated from this sweep's own measurements, with the same
            # exponent the panel's dotted continuation uses -- not from a table
            # of constants, which goes stale silently when the code changes.
            # Every curve can be extended this way, so unlike the table there is
            # no curve that quietly gets no substitution at all.
            power = spec.get("powers2", {}).get(name, 1)
            vals, sub = extrapolate_below_floor(
                Ns, [p[0] for p in memory[name]], power
            )
            if sub.any():
    ...
    # The computation alone, faded: solid markers and line are the total a user
    # must have free; the faded pair is what the arrays themselves need. The gap
    # between them is the runtime overhead, which is the point of showing both.
    for name, vals in mem_xla.items():
        ok = np.isfinite(vals)
        if not ok.any():
            continue
        ax2.plot(
            ...
            alpha=0.4,
            marker=markers[name],
            markersize=markersize[name] * 0.8,
            markerfacecolor="none",
            markeredgecolor=colors[name],
            markeredgewidth=1.2,
            zorder=3,
